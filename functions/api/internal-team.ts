...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    await sendEmail(env.RESEND_API_KEY, {
      subject: `Nueva solicitud de equipo interno – ${nombre}`,
      html: emailHtml('Nueva solicitud – Trabaja con Nosotros',
        row('Nombre', nombre) + row('Email', email) + row('Teléfono', telefono) + row('Motivación', mensaje)),
    });
    return jsonOk();
  } catch (e) {
    console.error(e);
